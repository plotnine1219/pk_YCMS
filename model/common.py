...
class C3WithCBAM(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super(C3WithCBAM, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(3 * c_, c2, 1)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.cbam = CBAM(c1, c_)  # 实例化 CBAM 注意力模块

    def forward(self, x):
        cbam_out = self.cbam(x)  # 使用 CBAM 注意力模块
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x), cbam_out), dim=1))
...
#         self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
#         self.cbam = CBAM(2 * c_, c2)  # Adding CBAM module
#
#     def forward(self, x):
#         feature1 = self.m(self.cv1(x))
#         feature2 = self.cv2(x)
#
#         # Apply CBAM module to feature1 and feature2
#         cbam_feature1 = self.cbam(feature1)
#         cbam_feature2 = self.cbam(feature2)
#
#         # Concatenate along the channel dimension
#         combined_features = torch.cat((cbam_feature1, cbam_feature2), dim=1)
#         out = self.cv3(combined_features)
#         return out

# # 实例化 C3WithCBAM
# c1 = 64  # 替换为你的输入通道数
# c2 = 128  # 替换为你的输出通道数
# n = 3  # 替换为你的 Bottleneck 层数
# shortcut = True  # 是否使用 shortcut
# g = 1  # 替换为你的分组数
# e = 0.5  # 替换为你的缩放因子
# c3_with_cbam = C3WithCBAM(c1, c2, n, shortcut, g, e)
#
# # 创建一个随机输入张量
# batch_size = 4
# input_channels = c1
...
#
# # 使用 C3WithCBAM 进行前向传播
# output = c3_with_cbam(input_tensor)
# print("Output shape:", output.shape)
#


# cbam_module = C3_CA(512,256)
